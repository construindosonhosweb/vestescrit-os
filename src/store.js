...
export function getData(){try{const raw=localStorage.getItem(KEY);if(raw)return JSON.parse(raw)}catch{} const data={products:seed(),categories,settings:{storeName:'VESTE',city:'Rio Branco',state:'Acre',heroTitle:'Seu estilo começa aqui.',heroText:'Descubra roupas, calçados e acessórios para transformar seu estilo em cada ocasião.',about:'Moda, qualidade e praticidade para acompanhar seu estilo.'}};saveData(data);return data}
...
export async function fetchRemoteData(){
  ...
  const hero=(content||[]).find(x=>x.key==='hero')?.value||{};
  const about=(content||[]).find(x=>x.key==='about')?.value||{};
  const fallback=getData();
  return {products:(rows||[]).map(p=>({id:p.id,name:p.name,category:catMap[p.category_id]||'Moda',price:Number(p.price),salePrice:p.sale_price==null?null:Number(p.sale_price),stock:p.stock,sku:p.sku||'',image:imageMap[p.id]||fallback.products[0]?.image,active:p.active,featured:p.featured,new:p.is_new})),categories:(cats||[]).map(c=>c.name),settings:{...fallback.settings,storeName:settings?.store_name||fallback.settings.storeName,city:settings?.city||'Rio Branco',state:settings?.state||'Acre',heroTitle:hero.title||fallback.settings.heroTitle,heroText:hero.description||fallback.settings.heroText,about:about.title||fallback.settings.about}};
}
